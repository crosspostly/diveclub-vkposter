...
**Разнесение путей загрузки видео (проверено живыми тестами):**

| Куда | Горизонталь | Вертикаль (клип) |
|---|---|---|
| Страница пользователя | обычный API (`video.save` + `wall.post`) | **тот же обычный API** — ВК сам делает клип (бубен не нужен) |
| Сообщество | обычный API (`video.save(group_id)` + `wall.post`) → видео в видеотеке | **только** `shortVideo.create` + токен официального приложения (бубен) |

Клип **в сообщество** грузится закрытым методом **`shortVideo.create`** —
он принимает только токены официального приложения ВК. Рабочих способа два:

1. **Web-токен из живой браузерной сессии (✅ единственный живой путь)** —
   перехват ответа `POST login.vk.ru/?act=web_token` (SPA сама его дёргает);
   токен 220 символов, **короткоживущий (~5–10 мин)**. Нужны права
   **редактора/админа** на группу, иначе error 15. Скрипт:
   **`clip_web_upload.py`**.
...
- Обычный флоу `video.save` + `wall.post` клип в сообществе **не создаёт** —
  ...
  раздел «Клипы» пуст. **Решает адресат, а не формат.**
