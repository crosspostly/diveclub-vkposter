...
- Клипы (Clips) в СООБЩЕСТВО через API НЕ загружаются: `shortVideo.create` → error 3 «Unknown method passed» (все версии API, любой токен); вертикальное видео через `video.save` + `wall.post` остаётся обычным видео (type=video, в разделе «Клипы» не появляется) — проверено и для 9:16, и для 3:4. Только через приложение/веб-интерфейс — см. `CLIPS.md`. Исключение: на ЛИЧНУЮ страницу пользователя вертикаль (в т.ч. 3:4) через `video.save` (без `group_id`) + `wall.post` становится клипом (attachment `type: short_video`)
